...
pub async fn amend(
    ...
    for (commit, pull_request) in pc.iter_mut().zip(pull_requests.into_iter()) {
        ...
        if let Some(pull_request) = pull_request {
            ...
            if opts.pull_code_changes && let Some(old_rev) = commit.message.get(&MessageSection::LastCommit) {
                jj.squash_copy(format!("commit_id({})..remote_bookmarks({}, {})", old_rev, pull_request.head.local(), config.remote_name).as_str(), crate::jj::ChangeId::from_str(commit.short_id.clone()))?;
            }
        }
